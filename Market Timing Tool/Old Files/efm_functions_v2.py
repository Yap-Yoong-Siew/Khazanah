...
def industry_group_weights(holdings):
    ...
    for date, group in holdings.groupby(level=0):
        ...
        for account, acct_group in group.groupby('Account Code'):
            sector_weights_dict[date][account] = {}
            account_sum = acct_group['Market Value'].sum()
            for sector, sector_group in acct_group.groupby('Industry Group_x'):
                sector_weight = sector_group['Market Value'].sum() / account_sum if account_sum != 0 else 0
                sector_weights_dict[date][account][sector] = sector_weight
    ...
    sector_weights_df['Industry Group Weight'] = sector_weights_df['Industry Group Weight'].fillna(0)
...
def plot_industry_group_weights(sector_weights_df, efm_dict, output_path):
    ...
    for efm in efm_dict.keys():
        ...
        df_filtered = sector_weights_df.loc[(sector_weights_df['Account Code'] == efm)]
        df_filtered.groupby('Industry Group')['Industry Group Weight'].plot(legend=True, figsize=(10, 6))
...
def tabulate_changes_ig(sector_weights_df, efm_dict, output_path, diff):
    ...
    for efm in efm_dict.keys():
        ...
        for sector in sector_weights_df['Industry Group'].unique():
            df_filtered = sector_weights_df[(sector_weights_df['Industry Group'] == sector) & (sector_weights_df['Account Code'] == efm)]
            if not df_filtered.empty and len(df_filtered) > diff:
                change = df_filtered['Industry Group Weight'].iloc[-1] - df_filtered['Industry Group Weight'].iloc[-diff-1]
                df_filtered_dict[efm_dict[efm]][sector] = change
...
def tabulate_changes_all_ig(sector_weights_df, efm_dict, output_path, change_windows):
    
    df_changes_list = []
    
    for diff in change_windows:
        sector_changes_df = tabulate_changes_ig(sector_weights_df, efm_dict, output_path, diff)
        df_styled = sector_changes_df.style.background_gradient(cmap='RdYlGn').format(format_pct).set_caption(f'Industry Group Changes changes in the last {diff} days')
        display(df_styled)
        df_changes_list.append(sector_changes_df)
        save_csv(sector_changes_df, output_path, f'sector_changes_{diff}_days')
